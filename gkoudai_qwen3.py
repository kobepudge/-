...
class Config:
    ...
    # 交易标的
    SYMBOL = "ag2512.SHFE"  # 默认主标的: 白银2025年12月合约
    # 多标的支持: 白银 + 碳酸锂 + 铝
    SYMBOLS = [
        "ag2512.SHFE",
        "lc2601.GFEX",
    ...
    CONTRACT_MULTIPLIER = {
        "ag2512.SHFE": 15,    # 15千克/手（平台有合约元数据时以平台为准）
        "lc2601.GFEX": 5,     # 5吨/手（示例值，平台优先）
    ...
    # API重试配置
    API_TIMEOUT = 90  # 拉长读超时，适配重量简报与并发排队
    API_MAX_RETRIES = 3
    ...
    # 执行稳健化：允许AI决策因等待而价格漂移时，按“保持R”为主重置止损，且限定最大追价
    EXEC_MAX_DRIFT_TICKS = 10    # 允许的最大漂移tick；超过则拒绝新仓
    EXEC_KEEP_R_FROM_AI = True   # 若AI提供 entry_price+stop_loss，优先保持该R
    EXEC_FAST_MOVE_ATR_RELAX = 0.6  # 快速趋势放宽：若漂移≤(ATR×此系数)/tick 则忽略 EXEC_MAX_DRIFT_TICKS 限制
    # —— 追踪止损稳健化参数（Phase 1/2/结构护栏）——
    # 启动门槛：仅当浮盈≥gate_R（单位：R）才启用追踪
    TRAILING_GATE_R = 0.8
    # 分阶段（盈≥gate_R 使用阶段1；盈≥2R 使用阶段2）。不同追踪类型取对应参数
    TRAILING_ATR_MULT_STAGE1 = 2.8
    TRAILING_ATR_MULT_STAGE2 = 2.0
    TRAILING_PERCENT_STAGE1 = 1.2  # %
    TRAILING_PERCENT_STAGE2 = 0.8  # %
    # 仅在1m收盘价触发追踪；并每 N 根1m刷新一次阈值
    TRAILING_TRIGGER_ON_CLOSE = True
...
class AIDecisionEngine:
    ...
    def call_deepseek_api(prompt, api_key: str = None):
        ...
        # 会话复用：为重量提示词与并发场景减小握手开销
        try:
            sess = getattr(AIDecisionEngine, '_session', None)
        except Exception:
            sess = None
        if sess is None:
            try:
                ...
                adapter = HTTPAdapter(pool_connections=8, pool_maxsize=8, max_retries=0, pool_block=True)
                sess.mount('https://', adapter)
                sess.mount('http://', adapter)
                # 记录到类属性
                AIDecisionEngine._session = sess
            except Exception:
                sess = requests
        ...
        for attempt in range(Config.API_MAX_RETRIES):
            try:
                # 区分连接与读取超时：连接10s、读取按配置
                _timeout = (10, float(getattr(Config, 'API_TIMEOUT', 90) or 90))
                post_obj = sess if hasattr(sess, 'post') else requests
                response = post_obj.post(
...
class TradeExecutor:
    ...
    def execute_decision(context, symbol, decision, tick, state):
        ...
        if signal == 'buy' and current_volume == 0:
            ...
            try:
                ...
                try:
                    ...
                    if ai_sl0 is not None:
                        spread_g = float(md_g.get('spread') or 0.0)
                        fric_ticks = max(1.0, (spread_g / max(1e-9, tksz)) + 1.0)
                        # 临时以盘口最佳价估计入场
                        est_entry = float(ask_price)
                        risk_ticks = abs(est_entry - float(ai_sl0)) / max(1e-9, tksz)
                        if risk_ticks > 0 and risk_ticks < (1.5 * fric_ticks):
            ...
            order_price = _align_price(tmp_price, 'buy')
            order_price = _normalize_price(order_price, tick_size, last_price)
            # 价格漂移处理：允许在可控范围内追价，并按“保持R”重置止损
            try:
                ai_entry = decision.get('entry_price')
                ai_sl = decision.get('stop_loss')
                if ai_entry is not None and tick_size and tick_size > 0:
                    drift_ticks = abs(float(order_price) - float(ai_entry)) / float(tick_size)
                    atr_val_g = float((state.get('last_market_data') or {}).get('atr') or 0.0)
                    fast_relax = (atr_val_g and (abs(float(order_price) - float(ai_entry)) <= (float(atr_val_g) * float(getattr(Config,'EXEC_FAST_MOVE_ATR_RELAX',0.6) or 0.6))))
                    if (drift_ticks > float(getattr(Config,'EXEC_MAX_DRIFT_TICKS',10) or 10)) and (not fast_relax):
                        Log(f"[{symbol}] 入场拒绝(buy): 价格漂移{drift_ticks:.1f}tick 超过上限")
                        return
                    # 保持R：以AI的(entry, sl)差值为基准重置sl，再交由护栏函数细化
                    if getattr(Config, 'EXEC_KEEP_R_FROM_AI', True) and ai_sl is not None:
                        try:
                            R_keep = max(0.0, float(ai_entry) - float(ai_sl))
                            decision['stop_loss'] = float(order_price) - R_keep
                        except Exception:
                            pass
            except Exception:
                pass
            ...
            order_price = _align_price(tmp_price, 'sell')
            order_price = _normalize_price(order_price, tick_size, last_price)
            # 价格漂移处理：允许在可控范围内追价，并按“保持R”重置止损（空头）
            try:
                ai_entry = decision.get('entry_price')
                ai_sl = decision.get('stop_loss')
                if ai_entry is not None and tick_size and tick_size > 0:
                    ...
                    fast_relax = (atr_val_g and (abs(float(order_price) - float(ai_entry)) <= (float(atr_val_g) * float(getattr(Config,'EXEC_FAST_MOVE_ATR_RELAX',0.6) or 0.6))))
                    if (drift_ticks > float(getattr(Config,'EXEC_MAX_DRIFT_TICKS',10) or 10)) and (not fast_relax):
                        Log(f"[{symbol}] 入场拒绝(sell): 价格漂移{drift_ticks:.1f}tick 超过上限")
                        return
                    if getattr(Config, 'EXEC_KEEP_R_FROM_AI', True) and ai_sl is not None:
                        try:
                            R_keep = max(0.0, float(ai_sl) - float(ai_entry))
                            decision['stop_loss'] = float(order_price) + R_keep
                        except Exception:
                            pass
            except Exception:
...
class RiskController:
    ...
    def check_and_enforce(context, symbol, tick, state):
        ...
        if state.get('ai_decision'):
            ...
            if ttype != 'none':
                # 追踪稳健化：延时启动(gate_R)、分阶段宽->窄、收盘触发、N根刷新、摆动位护栏
                try:
                    # 更新峰值/谷值（用于动态追踪与盈亏R）
                    if position_volume > 0:
                ...
                dc = state.get('data_collector')
                last_close = None; last_bar_ts = None
                try:
                    kbuf = getattr(dc, 'kline_1m_buffer', [])
                    if kbuf:
                        last_close = float(kbuf[-1]['close'])
                        last_bar_ts = getattr(kbuf[-1], 'datetime', None) or kbuf[-1].get('datetime')
                except Exception:
                    last_close = None; last_bar_ts = None
                use_price = float(last_close) if (getattr(Config, 'TRAILING_TRIGGER_ON_CLOSE', True) and last_close) else float(current_price)

                # 计算盈亏R并确定阶段
                try:
                    entry_px = float(state.get('position_avg_price') or 0)
                    sl0 = float(state.get('ai_decision', {}).get('stop_loss') or 0)
                except Exception:
                    entry_px = 0; sl0 = 0
                R = abs(entry_px - sl0) if (entry_px and sl0 and entry_px != sl0) else 0.0
                profit_R = 0.0
                if R > 0:
                    if position_volume > 0:
                        peak = float(state.get('peak_price') or use_price)
                        profit_R = max(0.0, (peak - entry_px) / R)
                    else:
                        trough = float(state.get('trough_price') or use_price)
                        profit_R = max(0.0, (entry_px - trough) / R)

                gate_R = float(getattr(Config, 'TRAILING_GATE_R', 0.8) or 0.8)
                stage = 0
                if profit_R < gate_R or R <= 0:
                    # 未达到门槛，不启用追踪
                    pass
                else:
                    stage = 1 if profit_R < 2.0 else 2
                # 刷新节拍：每 N 根1m 才更新一次 dyn_sl
                refresh_N = int(getattr(Config, 'TRAILING_REFRESH_BARS', 2) or 2)
                need_update = True
                try:
                    lt = (state.get('trailing') or {}).get('_last_update_ts')
                    li = (state.get('trailing') or {}).get('_last_update_idx', 0)
                    cur_idx = int(getattr(dc, '_kline_1m_index', 0) or 0)
                    if refresh_N > 1 and li and cur_idx and (cur_idx - int(li) < refresh_N):
                        need_update = False
                except Exception:
                    need_update = True

                dyn_sl = (state.get('trailing') or {}).get('_dyn_sl')
                if stage == 0:
                    # 不激活追踪，仅保留原dyn_sl（不触发），直到达到门槛
                    pass
                elif need_update:
                    md = state.get('last_market_data') if isinstance(state, dict) else None
                    atr_val = float((md.get('atr') if isinstance(md, dict) else 0) or 0)
                    tick_size = float(PlatformAdapter.get_pricetick(symbol) or 0.01)
                    low_20 = float((md.get('low_20') if isinstance(md, dict) else 0) or 0)
                    high_20 = float((md.get('high_20') if isinstance(md, dict) else 0) or 0)
                    buf_ticks = int(getattr(Config, 'TRAILING_BUFFER_TICKS', 3) or 3)
                    guard_low = (low_20 - buf_ticks * tick_size) if low_20 else None
                    guard_high = (high_20 + buf_ticks * tick_size) if high_20 else None

                    # 选择阶段参数
                    if ttype == 'atr':
                        mult1 = float(getattr(Config, 'TRAILING_ATR_MULT_STAGE1', 2.8) or 2.8)
                        mult2 = float(getattr(Config, 'TRAILING_ATR_MULT_STAGE2', 2.0) or 2.0)
                        eff_mult = mult1 if stage == 1 else mult2
                    else:
                        p1 = float(getattr(Config, 'TRAILING_PERCENT_STAGE1', 1.2) or 1.2)
                        p2 = float(getattr(Config, 'TRAILING_PERCENT_STAGE2', 0.8) or 0.8)
                        eff_pct = p1 if stage == 1 else p2

                    # 计算候选与护栏
                    if position_volume > 0:
                        peak = float(state.get('peak_price') or use_price)
                        candidates = []
                        if ttype == 'atr' and atr_val > 0:
                            candidates.append(peak - (eff_mult * atr_val))
                        if ttype == 'percent':
                            candidates.append(peak * (1 - eff_pct/100.0))
                        if candidates:
                            cand = max(candidates)  # 取更紧的那个（靠近价格）
                            if guard_low is not None:
                                cand = min(cand, guard_low)  # 不高于最近摆动低点-缓冲
                            # 不低于原始固定SL
                            try:
                                cand = max(float(stop_loss), cand)
                            except Exception:
                                pass
                            dyn_sl = cand
                    else:
                        ...
                        candidates = []
                        if ttype == 'atr' and atr_val > 0:
                            candidates.append(trough + (eff_mult * atr_val))
                        if ttype == 'percent':
                            candidates.append(trough * (1 + eff_pct/100.0))
                        if candidates:
                            cand = min(candidates)  # 取更紧的那个（靠近价格）
                            if guard_high is not None:
                                cand = max(cand, guard_high)  # 不低于最近摆动高点+缓冲
                            try:
                                cand = min(float(stop_loss), cand) if stop_loss else cand
                            except Exception:
                                pass
                            dyn_sl = cand
                    ...
                    # 记录更新节拍
                    try:
                        st_tr = state.get('trailing') or {}
                        st_tr['_dyn_sl'] = dyn_sl
                        st_tr['_stage'] = stage
                        st_tr['_last_update_ts'] = last_bar_ts or datetime.now()
                        # 简单的bar索引：使用k线缓冲长度
                        cur_idx = len(getattr(dc, 'kline_1m_buffer', []) or [])
                        st_tr['_last_update_idx'] = cur_idx
                        state['trailing'] = st_tr
                    except Exception:
                        pass

                # 触发：按收盘/或tick依据 use_price 对比
                if dyn_sl is not None:
                    if position_volume > 0 and use_price <= float(dyn_sl):
                        Log("[%s] 触发动态追踪止损(long): %.2f <= %.2f (stage=%s, gateR=%.2f, profitR=%.2f)" % (str(symbol), float(use_price), float(dyn_sl), str((state.get('trailing') or {}).get('_stage')), gate_R, profit_R))
                        send_target_order(symbol, 0)
                        state['ai_decision'] = None
                        state['position_avg_price'] = 0
                        return
                    if position_volume < 0 and use_price >= float(dyn_sl):
                        Log("[%s] 触发动态追踪止损(short): %.2f >= %.2f (stage=%s, gateR=%.2f, profitR=%.2f)" % (str(symbol), float(use_price), float(dyn_sl), str((state.get('trailing') or {}).get('_stage')), gate_R, profit_R))
                        send_target_order(symbol, 0)
                        state['ai_decision'] = None
                        state['position_avg_price'] = 0
                        return
...
def on_init(context):
    ...
    try:
        ...
        # 初始化资金：支持热参数（默认使用 Config.INITIAL_CASH）
        try:
            context.initial_cash = float(Config.INITIAL_CASH)
        except Exception:
            context.initial_cash = 2000000
        try:
            Log(f"初始资金: {float(context.initial_cash):.0f} 元")
        except Exception:
            pass
        ...
        try:
            ...
            if sym:
                ...
                rec = {
                    ...
                    'volume': vol,
                    'price': px,
                    'realized_delta': float(_delta_realized if tag.startswith('Close') else 0.0)
                }
                try:
